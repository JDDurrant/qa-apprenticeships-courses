...
-- Date:    August 2012
-- Purpose: Creates the database named CGDatabase that is required for City &
--          Guilds Develop Software using SQL assignment.  Note that the script
--          to create the database must have been executed before this script.
-------------------------------------------------------------------------------
...
Go
/* Foreign Key - Employees Manager ID -> Employees Employee No */
ALTER TABLE [dbo].[Employees] WITH CHECK
ADD CONSTRAINT FK_Employees_Employees_Manager_ID
FOREIGN KEY(Manager_ID) REFERENCES Employees(Employee_NO)

GO
/* Foreign Key - Departments Manager ID -> Employees Employee No */
ALTER TABLE Departments WITH CHECK
ADD CONSTRAINT FK_Departments_Employees_Manager_ID
FOREIGN KEY(Manager_ID) REFERENCES Employees(Employee_NO)
GO


Print 'Completed population of data tables.'
Go
